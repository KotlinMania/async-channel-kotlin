# Claude Code Project Instructions — async-channel-kotlin

## Project Overview

This is **async-channel-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`async-channel`](https://crates.io/crates/async-channel).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/async-channel/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/async-channel>

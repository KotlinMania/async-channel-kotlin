...
public sealed class TrySendError<T> {
    ...
    public class Full<T>(public val value: T) : TrySendError<T>() {
    ...
    public class Closed<T>(public val value: T) : TrySendError<T>() {
        override fun equals(other: Any?): Boolean {
            if (this === other) return true
    ...
    public fun intoInner(): T = when (this) {
        is Full -> value
        is Closed -> value
    }

    /** Returns `true` if the channel is full but not closed. */
    public fun isFull(): Boolean = when (this) {
        is Full -> true
        is Closed -> false
    }

    /** Returns `true` if the channel is closed. */
    public fun isClosed(): Boolean = when (this) {
        is Full -> false
        is Closed -> true
    }
}

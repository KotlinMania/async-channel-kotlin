...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AsyncChannel.swiftmodule/`
...
//      `import AsyncChannel` above would fail at compile time.
//
//   2. The static archive `libAsyncChannel.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AsyncChannel" }` and
//      `flattenPackage = "io.github.kotlinmania.asyncchannel"` configuration in
...
//      via the `AsyncChannelLibrary` product.
//
...
final class AsyncChannelExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AsyncChannel swift module imported cleanly")
    }
}
